...
CESMfile = '/Volumes/MyPassport/Data/Flooding/Physiology/cesm1_0_6.1850_prei.1deg.001.clm2.MonthlyConcat_Meg.nc';
lonDat = ncread(CESMfile,'lon');
latDat = ncread(CESMfile,'lat');
cesmRes = abs(latDat(2)-latDat(1)); 
...
for ibasin=1:nbasins
    ...
    for j=1:numel(NoDupeLat)
       k = find(latBasin>NoDupeLat(j)-cesmRes & latBasin<=NoDupeLat(j)+cesmRes); 
